...
from lib import competitors as competitors_mod, corpus, dates, discovery_handoff, env, freshness, html_render, http, mail, permission_preflight, pipeline, registers, render, schema, ui
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="JSON",
        help="Path to a prefetched inbox JSON (agent-produced via Gmail MCP) to "
             "fold in as a private first-party source, merged (inbox-flagged) "
             "into the ranked synthesis. See mail.py for the schema.",
    )
...
def _main(
    ...
    # Mail (fork addition): resolve the prefetched inbox JSON path the same way,
    # honoring EXCLUDE_SOURCES=mail, so availability/diagnose see it up front.
    resolved_mail_items = mail.resolve_items_path(
        args.mail_items, config.get("LAST30DAYS_MAIL_ITEMS")
    )
    if "mail" in excluded_sources:
        resolved_mail_items = None
    if resolved_mail_items:
